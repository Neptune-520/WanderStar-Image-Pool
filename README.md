# RandPic - 高性能随机图片 API 聚合器

## 项目概述
RandPic 是一个基于 Node.js + Express + SQLite 的高性能随机图片 API 聚合与分发系统。它旨在统一管理各种随机图床 API，提供自动熔断、权重优先级排序、统一鉴权、以及优雅的对内对外界面，将分散的图片 API 聚合成单一、高可用的大型图片服务。
